...
for i = 1:m,
  ...
  y_aux = zeros(num_labels, 1);
  y_aux(y(i)) = 1;
  a3 = sigmoid(a2*Theta2');
  J += (-log(a3)*y_aux-log(1-a3)*(1-y_aux));
end;

aux1 = ones(size(Theta1, 2), 1); aux1(1) = 0;
aux2 = ones(size(Theta2, 2), 1); aux2(1) = 0;
J = J / m + lambda*(sum(Theta1.^2*aux1) + sum(Theta2.^2*aux2)) / (2*m);
